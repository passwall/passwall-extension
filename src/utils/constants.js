export const TABS = [
  {
    name: 'Password',
    iconName: 'lock',
    createRouteName: 'PasswordCreate'
  },
  {
    name: 'Secure Notes',
    iconName: 'clipboard-secure',
    createRouteName: 'NoteCreate'
  },
  {
    name: 'Addresses',
    iconName: 'map-pin',
    createRouteName: 'AddressCreate'
  },
  {
    name: 'Payment Cards',
    iconName: 'credit-card',
    createRouteName: 'PaymentCardCreate'
  },
  {
    name: 'Bank Accounts',
    iconName: 'shield-check',
    createRouteName: 'BankAccountCreate'
  }
]

export const BROWSER_URL_PATTERNS = [/^chrome:\/\/.*/, /about:.*/]
export const PASSWALL_ICON_BS64 =
  'data:image/png;base64,'

/**
 * @enum {string}
 */
export const EVENT_TYPES = {
  TAB_UPDATE: 'TAB_UPDATE',
  REQUEST_PASSWORDS: 'REQUEST_PASSWORDS',
  GET_FILL_SECRET: 'GET_FILL_SECRET',
  GET_AUTOFILL_SECRET: 'GET_AUTOFILL_SECRET',
  REFRESH_TOKENS: 'REFRESH_TOKENS',
  LOGIN: 'LOGIN',
  LOGOUT: 'LOGOUT',
  SAVE_CREDENTIALS: 'SAVE_CREDENTIALS',
  SET_PENDING_SAVE: 'SET_PENDING_SAVE',
  CHECK_PENDING_SAVE: 'CHECK_PENDING_SAVE',
  CONFIRM_PENDING_SAVE: 'CONFIRM_PENDING_SAVE',
  DISMISS_PENDING_SAVE: 'DISMISS_PENDING_SAVE',
  SET_LAST_SEEN_USERNAME: 'SET_LAST_SEEN_USERNAME',
  GET_LAST_SEEN_USERNAME: 'GET_LAST_SEEN_USERNAME',
  UPDATE_LOGIN_USAGE: 'UPDATE_LOGIN_USAGE',
  SET_PENDING_TOTP: 'SET_PENDING_TOTP',
  GET_PENDING_TOTP: 'GET_PENDING_TOTP',
  CLEAR_PENDING_TOTP: 'CLEAR_PENDING_TOTP',
  // Password comparison (returns boolean only, never the stored password)
  CHECK_PASSWORD_MATCH: 'CHECK_PASSWORD_MATCH',
  // Form submission tracking
  GET_FORM_SUBMISSION_RESULT: 'GET_FORM_SUBMISSION_RESULT',
  FORM_SUBMISSION_SUCCESS: 'FORM_SUBMISSION_SUCCESS'
}
